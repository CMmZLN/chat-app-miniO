...
// ----------------
      ...
      (message) {
        try {
          ...
          if (parsedMessage.containsKey('message')) {
            ...
            if (messageContent != null &&
            ...
            } else if (messageContent.containsKey('direct_react_message')) {
              var reactmsg = messageContent['direct_react_message'];
              var userId = reactmsg['userid'];
              var directmsgid = reactmsg['directmsgid'];
              var emoji = reactmsg['emoji'];
              var reactUserInfo = messageContent['direct_reacted_user_info'];
              var emojiCount;
              bool emojiExists = false;
              for (var element in directEmojiCounts!) {
                if (element.directemoji == emoji &&
                    element.directmsgid == directmsgid) {
                  emojiCount = element.directemojiCounts! + 1;
                  element.directemojiCounts = emojiCount;
                  emojiExists = true;
                  break;
                }
              }
              if (!emojiExists) {
                emojiCount = 1;
                directEmojiCounts!.add(EmojiCountsforDirectMessage(
                    directmsgid: directmsgid,
                    directemoji: emoji,
                    directemojiCounts: emojiCount));
              }

              setState(() {
                if (emojiExists) {
                  directEmojiCounts!.add(EmojiCountsforDirectMessage(
                      directmsgid: directmsgid, directemojiCounts: emojiCount));
                }
                directReactUserDatas!.add(ReactUserDataForDirectMessage(
                    directMessageId: directmsgid,
                    emoji: emoji,
                    name: reactUserInfo,
                    userId: userId));
              });
            } else if (messageContent.containsKey('remove_direct_react')) {
              var deleteRection = messageContent['remove_direct_react'];
              ...
              setState(() {
                for (var element in directEmojiCounts!) {
                  if (element.directemoji == emoji &&
                      element.directmsgid == directmsgid) {
                    element.directemojiCounts = element.directemojiCounts! - 1;
                    break;
                  }
                }
                directReactUserDatas!.removeWhere((user) =>
                    user.emoji == emoji &&
                    user.directMessageId == directmsgid &&
                    user.name == reactUserInfo);
              });
            } else {
  ...
  Future<void> loadMessages() async {
    ...
    try {
      ...
      setState(() {
        ...
        isLoading = true;
        String filterMsg = thread.tDirectMessage!.directmsg!;
        directMessage = filterMsg;
        WidgetsBinding.instance.addPostFrameCallback((_) {
          _scrollToBottom();
        });
        tDirectThreads?.forEach((tDirectThread) {
          if (tDirectThread.draftMessageStatus == true &&
              tDirectThread.sendUserId == currentUserId) {
            insertEditText(tDirectThread.directthreadmsg);
          }
        });
      });
    } catch (e) {
  ...
  Future<void> sendReplyMessage(htmlcontent, draftStatus) async {
    ...
    if (htmlcontent.isNotEmpty || files.isNotEmpty) {
      await _directMessageService.sendDirectMessageThread(widget.directMsgId,
          widget.receiverId, htmlcontent, files, draftStatus);
      tDirectThreads?.forEach((tDirectThread) {
        if (tDirectThread.draftMessageStatus == true &&
            tDirectThread.sendUserId == currentUserId) {
          deleteReply(tDirectThread.id!);
        }
      });
      files.clear();
    }
  ...
  Widget build(BuildContext context) {
    if (SessionStore.sessionData!.currentUser!.memberStatus == false) {
      ...
      var filteredThreads = tDirectThreads
          ?.where((thread) =>
              ...
              onPressed: () {
                ...
                if (TextFieldContent.isNotEmpty) {
                  tDirectThreads?.forEach((tDirectThread) {
                    if (tDirectThread.draftMessageStatus == true &&
                        tDirectThread.sendUserId == currentUserId) {
                      draftedThreadId = tDirectThread.id;
                    }
                  });
                  if (draftedThreadId == null) {
                    ...
                    draftStatus = true;
                    sendReplyMessage(htmlContent, draftStatus);
                  } else {
                    htmlContent = detectStyles();
                    if (htmlContent.contains("<p>")) {
                      htmlContent = htmlContent.replaceAll("<p>", "");
                    ...
                    editdirectThreadMessage(htmlContent, draftedThreadId!);
                  }
                } else {
                  tDirectThreads?.forEach((tDirectThread) {
                    if (tDirectThread.draftMessageStatus == true &&
                                                        ...
                                                        return Container(
                                                          ...
                                                          child: TextButton(
                                                            onPressed:
                                                              ...
                                                              HapticFeedback
                                                                  .vibrate();
                                                              _directMessageService.directReactMsg(
                                                                  directEmojiCounts![
                                                                          index]
                                                                      .directemoji!,
                                                                  directMessageId!,
                        ...
                        Column(
                          children: [
                            Padding(
                              ...
                              child: Text(
                                '${filteredThreads!.length ?? 0} reply',
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            bool? draftMessageStatus =
                                tDirectThreads![index].draftMessageStatus;

                            if (tDirectThreads == null ||
                                tDirectThreads!.isEmpty ||
                            ...
                            if (index < filteredThreads.length) {
                              ...
                              String replyMessages = filterMsg;
                              ...
                              String time =
                                  filteredThreads[index].createdAt.toString();
                                                                ...
                                                                onPressed:
                                                                  ...
                                                                  await showModalBottomSheet(
                                                                      constraints: BoxConstraints(
                                                                          maxHeight: currentUserName == name
                                                                            ...
                                                                            children: [
                                                                              TextButton.icon(
                                                                                ...
                                                                                onPressed: () async {
                                                                                  ...
                                                                                  if (replyMessagesIds != null) {
                                                                                    if (isStar) {
                                                                                      await unStarReply(replyMessagesIds);
                                                                                    } else {
                                                                                      await starMsgReply(replyMessagesIds);
                                                                                    }
                                                                                  }
                                                                                },
                                                                              ),
                                                                                      ...
                                                                                      builder: (BuildContext context) {
                                                                                        return EmojiPicker(
                                                                                          onEmojiSelected: (category, Emoji emoji) async {
                                                                                            ...
                                                                                            await giveThreadMsgReaction(emoji: selectedEmoji, threadId: replyMessagesIds, selectedDirectMsgId: widget.directMsgId, selectedUserId: widget.receiverId, userId: currentUserId);
                                                                              ...
                                                                              if (currentUserName == name)
                                                                                TextButton.icon(
                                                                                    ...
                                                                                    onPressed: () {
                                                                                      ...
                                                                                      editMsg = replyMessages;
                                                                                      editMsgId = replyMessagesIds;
                                                                              ...
                                                                              if (currentUserName == name)
                                                                                TextButton.icon(
                                                                                  label: const Text("Delete Message", style: TextStyle(fontSize: 20, color: Colors.red)),
                                                                                  onPressed: () async {
                                                                                    AuthService.checkTokenStatus(context);
                                                                                    await deleteReply(replyMessagesIds);
                                                                                    Navigator.pop(context);
                                                                                  },
